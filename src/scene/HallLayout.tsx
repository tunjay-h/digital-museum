...
const HallLayout = () => {
  ...
  const { wallTexture, floorTexture, ceilingTexture } = useHallTextures();
  ...
  return (
    <group>
      <mesh position={[0, 0, -hallLength / 2]} rotation={[-Math.PI / 2, 0, 0]}>
        ...
        <meshBasicMaterial map={floorTexture} color="#2b211a" />
      </mesh>
      <mesh position={[0, 3, -hallLength / 2]} rotation={[Math.PI / 2, 0, 0]}>
        <planeGeometry args={[CORRIDOR_WIDTH, hallLength + 8]} />
        <meshBasicMaterial map={ceilingTexture} color="#14161c" />
      </mesh>
      <mesh position={[-CORRIDOR_WIDTH / 2, 2.1, -hallLength / 2]} rotation={[0, Math.PI / 2, 0]}>
        <planeGeometry args={[hallLength + 8, 5]} />
        <meshBasicMaterial map={wallTexture} side={DoubleSide} color="#2a241f" />
      </mesh>
      <mesh position={[CORRIDOR_WIDTH / 2, 2.1, -hallLength / 2]} rotation={[0, -Math.PI / 2, 0]}>
        <planeGeometry args={[hallLength + 8, 5]} />
        <meshBasicMaterial map={wallTexture} side={DoubleSide} color="#2a241f" />
      ...
      <mesh position={[0, 2.1, END_Z - 2]} rotation={[0, Math.PI, 0]}>
        ...
        <meshBasicMaterial map={wallTexture} side={DoubleSide} color="#2e251f" />
      </mesh>
